...
define("LINKDIR_VER","2.1.1");
define("LINKDIR_DB_VER","1.1");

if(isset($_GET['linkdir'])) {
	...
	x('<div class="linkdir_block" id="linkdir">');
		...
		if(TYPE=="addnew"){
			if(get_option("allow_new_links")==1):
				...
				if(isset($success) && ($success_msg != '')){
					x('<p class="linkdir_success">Your URL has been submitted. Thanks!</p>');
				}
			...
			if(cat_exists()){
				...
				$page = 1;
				if(isset($_GET['_page']) && is_numeric($_GET['_page']) && $_GET['_page']>0 && $_GET['_page']<=$pages) {
					$page = floor($_GET['_page']);
				}
...
{	
	...
	$sql = $wpdb->prepare('CREATE TABLE IF NOT EXISTS %s (
	  ...
	  PRIMARY KEY (`id`),
	  UNIQUE KEY `url` (`url`)
	) ENGINE=InnoDB  DEFAULT CHARSET=utf8;', LINKDIR_TABLE);
	...
	$sql = $wpdb->prepare('CREATE TABLE IF NOT EXISTS %s (
	  ...
	  `description` varchar(250) NOT NULL,
	  PRIMARY KEY (`id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8 ;', LINKDIR_TABLE_CATS);
	...
	$sql = $wpdb->prepare('CREATE TABLE IF NOT EXISTS %s (
	  ...
	  KEY `time` (`time`),
	  KEY `for` (`for`)
	) ENGINE=InnoDB  DEFAULT CHARSET=utf8;', LINKDIR_TABLE_HITS);
...
function linkdir_blacklists(){
	...
	if(count($forbidden_ips_data)>0){
		foreach($forbidden_ips_data as $ip){
			$forbidden_ips .= $ip."\n";
		}
	}

	if(count($forbidden_domains_data)>0){
		foreach($forbidden_domains_data as $domain){
			$forbidden_domains .= $domain."\n";
		}
	}
